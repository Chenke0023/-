#!/usr/bin/env python3
"""从 ai-rss-filter 数据库读取并批量过滤新闻 - Render 版本"""
...
'''
    ...
    # 保存到 /tmp 目录（Render 上可写）
    output_dir = "/tmp"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    json_file = os.path.join(output_dir, f"filtered_news_{timestamp}.json")
    md_file = os.path.join(output_dir, f"相关新闻_{timestamp}.md")
